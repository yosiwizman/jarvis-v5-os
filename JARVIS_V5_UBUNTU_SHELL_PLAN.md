# Jarvis V5 OS – Ubuntu Shell Integration Plan

This document defines how Jarvis V5 will run as a branded, full-screen "shell" experience on Ubuntu Linux while maintaining proper OS-level security and user experience.
...
This plan describes how **Jarvis V5 OS** will be deployed on Ubuntu as a full-screen, kiosk-style shell that replaces the traditional desktop environment from a user's perspective, while still leveraging Ubuntu's native security features.

**Key principles:**

- **Ubuntu remains the underlying OS**: Jarvis is a branded experience layer, not a replacement for the operating system.
- **Leverage Ubuntu's security**: We use Ubuntu's login, lock screen, and account management as the primary security boundary.
- **Full-screen branded experience**: Users see only Jarvis UI, not the Ubuntu desktop, panels, or traditional window manager.
...
Instead, Jarvis V5 on Ubuntu is a **curated shell experience** designed for AI workstations, factory rigs, and dedicated Jarvis devices, where the full power of Ubuntu is present but the UX is streamlined and branded.

---

## 2. Operating Modes (CTO Decision)

Jarvis V5 will support **two deployment modes** to accommodate different use cases. **Secure Workstation Mode** is the **recommended default** for most users.
...
- After login, Jarvis auto-launches in full-screen (kiosk mode).
- Traditional Ubuntu desktop is hidden; users interact only with Jarvis UI.
- Optional: Jarvis may implement its own PIN-locked overlay for convenience (future feature).

**Security model:**

- **Primary security**: Ubuntu account password + disk encryption (LUKS).
- **Secondary security** (future): Jarvis PIN lock overlay for quick locking without triggering full OS lock.
...
1. Boot → GDM login screen (themed with Jarvis branding).
2. User enters Ubuntu password → session starts.
3. Jarvis launches full-screen automatically.
4. User sees only Jarvis UI (no desktop, no panels).
5. Lock (Super+L) → Ubuntu lock screen → unlock with password → Jarvis resumes.
...
- Ubuntu **auto-login** enabled for a dedicated service account (e.g., `jarvis-kiosk`).
- Jarvis launches full-screen on boot without user interaction.
- No visible Ubuntu desktop or login screen.
- Security relies on **physical access controls**, **network isolation**, and **Jarvis's internal PIN lock** (when implemented).

**Security model:**

- **Primary security**: Physical access + network controls.
- **Secondary security** (future): Jarvis PIN lock overlay.
...
1. Boot → auto-login to `jarvis-kiosk` account.
2. Jarvis launches full-screen immediately.
3. User sees only Jarvis UI.
4. Lock (if enabled) → Jarvis PIN overlay (future feature).
...
3. **GDM login screen appears**:
   - Background: Jarvis-themed wallpaper (dark, branded).
   - Logo: Jarvis logo visible.
   - User enters Ubuntu password.
4. **Session starts**:
   - Ubuntu initializes user session.
   - Auto-start script launches Jarvis in full-screen mode (browser or Electron).
   - Traditional desktop/panels are hidden or disabled.
5. **Jarvis UI loads**:
   - User sees Jarvis dashboard at `https://localhost:3000`.
   - No visible browser chrome, URL bar, or tabs.
   - Full-screen, branded experience.

**Expected time:** ~10-30 seconds from power-on to Jarvis ready (depending on hardware and SSD speed).
...
1. User presses **Super+L** (or clicks a lock button if added to Jarvis UI).
2. Ubuntu lock screen appears immediately.
3. Browser/Electron app remains running in background.
4. Lock screen shows:
   - Jarvis-themed background.
   - Password prompt.
5. User enters Ubuntu password → session unlocks.
6. Jarvis UI reappears, exactly as it was before lock.
...
### 3.3 Future: Jarvis Lock Overlay (Phase B)

**Scenario:** User wants to quickly lock Jarvis without triggering the full OS lock screen.

**Flow:**

1. User clicks **"Lock Jarvis"** button in UI (e.g., in settings or a quick-action menu).
2. Jarvis displays a **full-screen PIN overlay**:
   - Branded background (Jarvis logo, dark theme).
...
- Jarvis can auto-lock after inactivity (configurable in settings).
- Same unlock flow as manual lock.

**Security note:** This is a **convenience layer**, not a replacement for Ubuntu's lock screen. A technically savvy user could still bypass it by switching to a different TTY or killing the Jarvis process. For real security, use Ubuntu's lock screen (Super+L) or Secure Workstation Mode.
...
1. **Boot** → Ubuntu auto-logs in to `jarvis-kiosk` account.
2. Auto-start script launches Jarvis full-screen.
3. User sees Jarvis UI immediately (no login prompt).
4. Lock (if future Jarvis PIN is implemented) → Jarvis PIN overlay → unlock with PIN.
...
**Goal:** Make Jarvis launch full-screen on Ubuntu login, with minimal visible desktop elements.

**Scope:**

- Keep Ubuntu login and lock screen **enabled** (Secure Workstation Mode).
- Theme GDM login/lock screen with Jarvis branding.
- Auto-start Jarvis in full-screen kiosk mode after login.
- Hide traditional Ubuntu desktop elements (panels, dock, etc.).

**Tasks:**

1. **Theme GDM login and lock screen:**
   - Replace default Ubuntu wallpaper with Jarvis-branded background (dark, logo).
...
2. **Choose kiosk technology:**
   - **Option A: Full-screen browser (Chromium/Chrome in kiosk mode)**:
     ...
     - Enable auto-reconnect if Jarvis server restarts.
   - **Option B: Electron app wrapper**:
     - Package Jarvis UI in an Electron app.
...
3. **Auto-start Jarvis on login:**
   ...
   - Example: `jarvis-kiosk.desktop`:
     ...
     ```
   - Ensure Jarvis server (Fastify backend) is also auto-started via systemd service.
...
5. **Systemd service for Jarvis backend:**
   - Create `/etc/systemd/system/jarvis-server.service`:
     ```ini
     [Unit]
     Description=Jarvis V5 Backend Server
     After=network.target

     [Service]
     Type=simple
     User=jarvis
     WorkingDirectory=/opt/jarvis-v5
     ExecStart=/usr/bin/node /opt/jarvis-v5/apps/server/dist/index.js
     Restart=always
     RestartSec=5
     ...
     ```
   - Enable and start: `systemctl enable jarvis-server && systemctl start jarvis-server`.
...
7. **Test and validate:**
   ...
   - Log in → verify Jarvis launches full-screen automatically.
   - Verify Super+L locks correctly and unlock resumes Jarvis.
   - Verify no desktop elements visible.

**Deliverables (Phase A):**

- Themed GDM login/lock screen.
- Auto-start configuration for Jarvis (browser or Electron).
- Systemd service for Jarvis backend.
...
### Phase B – Jarvis PIN Lock (In-UI Security Layer)

**Goal:** Add a convenient, branded PIN lock overlay inside Jarvis UI.

**Scope:**

- Implement a React component for full-screen PIN entry overlay.
- Store PIN securely (hashed, encrypted) in Jarvis settings.
- Support manual "Lock Jarvis" action and auto-lock after inactivity.
- Integrate with existing Jarvis UI theme and branding.
...
2. **Backend: PIN storage and validation:**
   - Add `jarvisPinHash` field to settings (SHA-256 hash of PIN).
...
4. **Manual lock action:**
   - Add "Lock Jarvis" button in settings or quick-action menu.
...
- Bridge Ubuntu notifications into Jarvis notification center.
...
- System-level keyboard shortcuts routed to Jarvis.
...
1. **Custom GDM greeter:**
   ...
   - Show Jarvis branding, logo, and custom animations on login screen.

2. **Notification integration:**
   - Listen to Ubuntu's notification daemon (D-Bus).
   - Forward notifications to Jarvis notification center.
   - Allow Jarvis to trigger system notifications.
...
3. **Biometric unlock:**
   ...
   - Voice unlock via Jarvis's existing speech recognition.

4. **Wayland session:**
   - Investigate running Jarvis as a Wayland compositor client.
   - Better security (X11 allows keylogging between apps).
   - Smoother full-screen rendering.

5. **Keyboard shortcut integration:**
   - Register global shortcuts (Super+J, etc.) to trigger Jarvis actions.
...
**Jarvis does NOT replace these.** Ubuntu's security is the foundation.

### Secondary Security: Jarvis PIN Lock (Future)

- **Jarvis PIN** is a **convenience layer** for quick locking.
- It does **NOT** protect against:
  - TTY access (Ctrl+Alt+F2, etc.).
  - Process termination (kill Jarvis process).
  - Physical access to the machine.
- It **DOES** protect against:
  - Casual snooping when user steps away briefly.
  - Accidental UI access by other users in the room.

**Use case:** User is in a shared office and wants to lock Jarvis while leaving Ubuntu session running (e.g., ongoing download or server process). They use Jarvis PIN lock instead of full OS lock.
...
- **For convenience:** Enable Jarvis PIN lock once Phase B is complete.
...
- Jarvis V5 core features are stable and tested on development platforms (macOS, Windows WSL, etc.).
...
1. **Core Jarvis features are stable** ✅ (already true as of v5.9.0).
...
- **Continue developing Jarvis features** on current platforms (macOS, WSL) in parallel.
- **Dedicate a separate Ubuntu workstation** or VM for shell integration work.
- **Use feature flags** or environment variables to enable Ubuntu-specific features (e.g., `JARVIS_UBUNTU_MODE=kiosk`).

### Version planning

- **Phase A (Ubuntu Shell Integration)** will be released as **v6.0.0** or **v6.1.0**.
- **Phase B (Jarvis PIN Lock)** will be released as **v6.2.0** or later.
...
- Update `JARVIS_V5_REPO_OVERVIEW.md` to mention Ubuntu shell mode.
- Create installation guide: `UBUNTU_INSTALLATION_GUIDE.md`.
- Create troubleshooting guide: `UBUNTU_TROUBLESHOOTING.md`.
- Update `JARVIS_V5_TEST_PLAN.md` to include Ubuntu-specific test cases.

---

## 7. Summary

This document defines the architecture and migration plan for running **Jarvis V5 OS as a branded shell on Ubuntu**.
...
- **Implementation is phased**:
  ...
  - **Phase B**: Jarvis PIN lock overlay (v6.2.0+).
