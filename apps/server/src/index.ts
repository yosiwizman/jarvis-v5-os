...
fastify.get("/api/integrations/gmail/inbox", async (req, reply) => {
  ...
  try {
    ...
    return reply
      .status(502)
      .send({ ok: false, error: "gmail_inbox_fetch_failed" });
  }
});

// Gmail fetch full message endpoint
fastify.get(
  "/api/integrations/gmail/message/:messageId",
  async (req, reply) => {
    ...
    if (!messageId) {
      return reply
        .status(400)
        .send({ ok: false, error: "message_id_required" });
    }
    ...
    try {
      ...
      return reply
        .status(502)
        .send({ ok: false, error: "gmail_message_fetch_failed" });
    }
  },
);
...
fastify.post("/api/integrations/gmail/send", async (req, reply) => {
  ...
  if (!body.to || !body.subject || !body.body) {
    return reply
      .status(400)
      .send({ ok: false, error: "to_subject_body_required" });
  }
...
fastify.get(
  "/api/integrations/google-calendar/sync-events",
  async (req, reply) => {
    // Load settings
    let settings: any = null;
    try {
      ...
      logger.error(
        { error },
        "Failed to load settings for Google Calendar events",
      );
      return reply
        .status(500)
        .send({ ok: false, error: "failed_to_load_settings" });
    ...
    const maxResults = Math.max(
      1,
      Math.min(50, limit ? parseInt(limit, 10) : 10),
    );
    ...
    try {
      ...
      return reply
        .status(502)
        .send({ ok: false, error: "calendar_events_fetch_failed" });
    }
  },
);
...
fastify.post(
  "/api/integrations/google-calendar/sync-reminders",
  async (req, reply) => {
    ...
    try {
      ...
      if (!result.ok || !result.events) {
        ...
        return reply
          .status(502)
          .send({ ok: false, error: "failed_to_fetch_events" });
      }
      ...
      for (const event of result.events) {
        ...
        try {
          ...
          if (reminderTime.getTime() > Date.now()) {
            ...
            scheduledCount++;
            logger.info(
              { eventId: event.id, summary: event.summary, reminderTime },
              "Scheduled calendar reminder",
            );
          }
        } catch (scheduleError) {
          logger.error(
            { error: scheduleError, eventId: event.id },
      ...
      logger.info(
        { eventsFound: result.events.length, scheduledCount },
        "Calendar reminder sync complete",
      );
      ...
      return reply
        .status(502)
        .send({ ok: false, error: "calendar_sync_failed" });
    }
  },
);
...
fastify.post("/api/integrations/google-calendar/test", async (req, reply) => {
  ...
  try {
    ...
    return reply
      .status(502)
      .send({ ok: false, error: "google_calendar_test_failed" });
  }
});
...
fastify.get("/api/email-notifications/status", async (req, reply) => {
  try {
    const { getEmailNotificationChecker } =
      await import("./integrations/email-notifications.js");
...
fastify.post("/api/email-notifications/trigger", async (req, reply) => {
  try {
    ...
    if (!checker) {
      return reply
        .status(503)
        .send({ ok: false, error: "email_notifications_not_initialized" });
    }
    ...
    return reply
      .status(500)
      .send({ ok: false, error: "failed_to_trigger_check" });
  }
});
...
fastify.post("/api/email-notifications/config", async (req, reply) => {
  try {
    ...
    return reply
      .status(500)
      .send({ ok: false, error: "failed_to_update_config" });
  }
});
