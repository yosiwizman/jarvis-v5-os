...
const MOCK_TAILSCALE =
  process.env.MOCK_TAILSCALE === "true" || process.env.CI === "true";
...
async function runTailscaleUp(
  ...
  return new Promise((resolve) => {
    ...
    exec(
      `tailscale up ${args}`.trim(),
      { timeout: 60000 },
      (error, stdout, stderr) => {
        if (error) {
          // SECURITY: Redact any potential authkey from error messages
          const safeError = (stderr || error.message).replace(
            /--authkey=\S+/g,
            "--authkey=[REDACTED]",
          );
...
async function enableTailscaleServe(
  ...
  return new Promise((resolve) => {
    ...
    exec(command, { timeout: 30000 }, (error, stdout, stderr) => {
      ...
      const url = tailscale.magicDnsName
        ? `https://${tailscale.magicDnsName}/`
        : undefined;
 ...
 */
async function disableTailscaleServe(): Promise<{
  ok: boolean;
  error?: string;
}> {
  if (MOCK_TAILSCALE) {
    ...
    exec(
      ...
      (error, stdout, stderr) => {
        if (error) {
          ...
          logger.debug(
            { error: stderr || error.message },
            "tailscale serve off (may be already off)",
          );
        }
...
export function registerRemoteAccessRoutes(fastify: FastifyInstance) {
   ...
   */
  fastify.get(
    "/api/admin/remote-access/status",
   ...
   */
  fastify.post(
    "/api/admin/remote-access/enable",
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!parsed.success) {
        return reply.status(400).send({
          ...
          message:
            'Invalid request body. Expected { mode: "tailscale", authKey?: string }',
        });
      }
      ...
      logger.info(
        { mode, hasAuthKey: !!authKey },
        "Remote access enable requested",
      );
      ...
      if (!tailscale.installed) {
        return reply.status(400).send({
          ...
          suggestion:
            "Install Tailscale from https://tailscale.com/download and try again.",
        });
      }
      ...
      if (!updatedTailscale.up) {
        return reply.status(400).send({
          ...
          suggestion:
            'Provide an auth key or run "tailscale up" manually first.',
        });
      }
   ...
   */
  fastify.post(
    "/api/admin/remote-access/disable",
    async (req: FastifyRequest, reply: FastifyReply) => {
      if (!requireAdmin(req, reply)) return;
      ...
      if (!serveResult.ok) {
        logger.warn(
          ...
          "Failed to disable serve (may be already off)",
        );
      }
