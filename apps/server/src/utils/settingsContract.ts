...
const AzureTTSConfigSchema = z.object({
  enabled: z.boolean().optional(),
  apiKey: z.string().nullable().optional(),
...
const AlexaConfigSchema = z.object({
  enabled: z.boolean().optional(),
  clientId: z.string().nullable().optional(),
  clientSecret: z.string().nullable().optional(),
  refreshToken: z.string().nullable().optional(),
...
const IRobotConfigSchema = z.object({
  enabled: z.boolean().optional(),
...
const SmartLightsConfigSchema = z.object({
  enabled: z.boolean().optional(),
...
export const RawSettingsSchema = z.object({
  ...
  imageGeneration: z.object({
    ...
    partialImages: z.union([z.literal(0), z.literal(1), z.literal(2), z.literal(3)]).optional()
  }).passthrough().optional(),
...
const defaultIntegrations = {
  weather: { enabled: false, provider: 'openweather' as const, defaultLocation: 'Miami,US' },
  webSearch: { enabled: false, baseUrl: null, apiKey: null, defaultRegion: null },
  localLLM: { enabled: false, provider: 'ollama' as const, baseUrl: 'http://localhost:11434', apiKey: null, model: 'llama3.1', temperature: 0.7, maxTokens: null },
  elevenLabs: { enabled: false, apiKey: null, voiceId: null, modelId: 'eleven_multilingual_v2', stability: null, similarityBoost: null, style: null },
  azureTTS: { enabled: false, apiKey: null, region: 'eastus', voiceName: 'en-US-JennyNeural', style: null, rate: null, pitch: null },
  spotify: { enabled: false, clientId: null, clientSecret: null, defaultMarket: 'US' },
  gmail: { enabled: false, clientId: null, clientSecret: null, redirectUri: null, refreshToken: null, userEmail: null },
  googleCalendar: { enabled: false, clientId: null, clientSecret: null, redirectUri: null, refreshToken: null, calendarId: 'primary' },
  alexa: { enabled: false, clientId: null, clientSecret: null, refreshToken: null, region: 'NA' },
  irobot: { enabled: false, username: null, password: null, robotId: null },
  nest: { enabled: false, projectId: null, clientId: null, clientSecret: null, refreshToken: null, deviceId: null },
  smartLights: { enabled: false, provider: 'hue' as const, apiKey: null, bridgeIp: null }
};
...
const defaultSettings = {
  jarvis: {
    ...
    voice: 'echo',
    initialPrompt: 'You are AKIOR (Advanced Knowledge & Intelligence Operating Resource), a sophisticated AI assistant.',
    hotword: 'akior',
...
function normalizeIntegrations(input: any): typeof defaultIntegrations {
  ...
  return {
    ...
    elevenLabs: { ...defaultIntegrations.elevenLabs, ...(input.elevenLabs ?? {}) },
    ...
    googleCalendar: { ...defaultIntegrations.googleCalendar, ...(input.googleCalendar ?? {}) },
    ...
    smartLights: { ...defaultIntegrations.smartLights, ...(input.smartLights ?? {}) }
  };
}
 ...
 */
export function normalizeServerSettings(input: unknown): typeof defaultSettings {
  const partial = (input && typeof input === 'object' ? input : {}) as Record<string, any>;
 ...
 */
export function validateAndNormalizeSettings(rawJson: unknown): ContractValidationResult {
 ...
 */
export function safeJsonParse(jsonString: string): { data: unknown; error?: string } {
  try {
