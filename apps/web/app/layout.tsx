      ...
      <body className="min-h-screen relative">
        <ThemeProvider>
          ...
          <nav className="space-y-2 flex-1">
            <Link className="block btn truncate" href="/menu" title="Menu">
              {isCollapsed ? 'M' : 'Menu'}
            </Link>
            <Link className="block btn truncate" href="/jarvis" title="Jarvis">
              {isCollapsed ? 'J' : 'Jarvis'}
            </Link>
            <Link className="block btn truncate" href="/3dmodel" title="3D Model">
              {isCollapsed ? '3D' : '3D Model'}
            </Link>
            <Link className="block btn truncate" href="/3dViewer" title="3D Viewer">
              {isCollapsed ? '3V' : '3D Viewer'}
            </Link>
            <Link className="block btn truncate" href="/createimage" title="Create Image">
              {isCollapsed ? 'I' : 'Create Image'}
            </Link>
            <Link className="block btn truncate" href="/3dprinters" title="3D Printers">
              {isCollapsed ? '3P' : '3D Printers'}
            </Link>
            <Link className="block btn truncate" href="/files" title="Files">
              {isCollapsed ? 'F' : 'Files'}
            </Link>
            <Link className="block btn truncate" href="/chat" title="Chat">
              {isCollapsed ? 'C' : 'Chat'}
            </Link>
            <Link className="block btn truncate" href="/security" title="Security">
              {isCollapsed ? 'S' : 'Security'}
            </Link>
            <Link className="block btn truncate" href="/camera" title="Camera">
              {isCollapsed ? 'Ca' : 'Camera'}
            </Link>
            {/* Expose Holomat apps deck explicitly in the sidebar (adds UI route without changing existing links) */}
            <Link className="block btn truncate" href="/holomat" title="Holomat">
              {isCollapsed ? 'Ho' : 'Holomat'}
            </Link>
            <Link className="block btn truncate" href="/functions" title="Functions">
              {isCollapsed ? 'Fn' : 'Functions'}
            </Link>
            <Link className="block btn truncate" href="/settings" title="Settings">
