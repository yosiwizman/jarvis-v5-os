...
type FeedbackState = Partial<
  Record<KeyName, { type: "success" | "error"; message: string }>
>;
...
export default function SettingsPage() {
  ...
  const [pendingKeys, setPendingKeys] = useState<Record<KeyName, string>>({
    openai: "",
    meshy: "",
  });
  const [savingKeys, setSavingKeys] = useState<Record<KeyName, boolean>>({
    openai: false,
    meshy: false,
  });
  ...
  const [ollamaModels, setOllamaModels] = useState<
    Array<{ name: string; parameterSize: string | null }>
  >([]);
  ...
  const [activeMemoryTab, setActiveMemoryTab] = useState<
    "conversations" | "actions" | "logs"
  >("conversations");
  const [bambuAuthMessage, setBambuAuthMessage] = useState("");

  // Smart Home integration test state
  const [smarthomeTestResults, setSmarthomeTestResults] = useState<
  ...
  // Fetch available Ollama models when localLLM integration is enabled
  useEffect(() => {
    if (!settings?.integrations?.localLLM?.enabled) return;
    if (settings?.integrations?.localLLM?.provider !== "ollama") return;
    setOllamaLoading(true);
    setOllamaError(null);
    fetch(buildServerUrl("/api/integrations/ollama/models"))
      .then((res) =>
        res.ok ? res.json() : Promise.reject(new Error("Ollama unreachable")),
      )
      .then((data) => {
      ...
      .catch((err) => setOllamaError(err.message || "Failed to fetch models"))
      .finally(() => setOllamaLoading(false));
  }, [
    settings?.integrations?.localLLM?.enabled,
    settings?.integrations?.localLLM?.provider,
  ]);
  ...
  function updateModel<K extends keyof AppSettings["models"]>(
    key: K,
    value: AppSettings["models"][K],
  ) {
  ...
  function updateTextChat<K extends keyof TextChatSettings>(
    key: K,
    value: TextChatSettings[K],
  ) {
  ...
  async function saveKey(name: KeyName) {
    ...
    if (!value) {
      setFeedback((prev) => ({
        ...prev,
        [name]: { type: "error", message: "Enter a key before saving." },
      }));
      return;
    }
    ...
    try {
      ...
      setFeedback((prev) => ({
        ...prev,
        [name]: { type: "success", message: "Key saved." },
      }));
    } catch (error) {
      const csrfMsg =
  ...
  async function removeKey(name: KeyName) {
    ...
    setFeedback((prev) => ({ ...prev, [name]: undefined }));
    try {
      const response = await apiFetch(buildServerUrl(`/admin/keys/${name}`), {
        method: "DELETE",
      });
      let payload: any = null;
      try {
      ...
      setFeedback((prev) => ({
        ...prev,
        [name]: { type: "success", message: "Key removed." },
      }));
    } catch (error) {
      const csrfMsg =
        error instanceof CsrfError
  ...
  const handleBambuLogin = async () => {
    ...
    try {
      ...
      setBambuAuthStatus("error");
      setBambuAuthMessage(
        csrfMsg || (err instanceof Error ? err.message : "Login error"),
      );
    }
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      setBambuAuthStatus("error");
      setBambuAuthMessage(
        csrfMsg || (err instanceof Error ? err.message : "Verification error"),
      );
    }
  ...
  const handleToggleLockdown = async () => {
    ...
    try {
      ...
      const endpoint = lockdownState.active
        ? "/api/lockdown/deactivate"
        : "/api/lockdown/activate";
      ...
      if (!response.ok) {
        ...
        alert(
          `Failed to ${lockdownState.active ? "deactivate" : "activate"} lockdown: ${error.error || "Unknown error"}`,
        );
        return;
      }
      ...
      alert(
        `Failed to toggle lockdown: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  };

  // Smart Home integration test handlers
  const handleTestSmartHome = async (
    integration: "alexa" | "irobot" | "nest" | "smartLights",
  ) => {
  ...
  const hasDrift =
    serverBuild && clientSha !== "unknown" && serverSha !== clientSha;
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        <div className="flex items-center gap-3 text-xs">
          <div className="flex items-center gap-2">
            ...
            <code
              className={`font-mono px-2 py-0.5 rounded ${hasDrift ? "bg-amber-500/20 text-amber-400" : "bg-white/5 text-cyan-400"}`}
      ...
      {hasDrift && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 space-y-2">
          ...
          <p className="text-sm text-white/60">
            Your browser loaded client code from{" "}
            <code className="text-cyan-400">{clientSha}</code> but the server is
            running <code className="text-cyan-400">{serverSha}</code>. This may
      ...
      <section className="card p-6 space-y-3">
        <header className="flex items-center justify-between">
          <div>
            ...
            <div className="text-white/60 text-sm">
              Configure LLM providers, HTTPS certificates, and API keys
            </div>
          </div>
          <a
            href="/setup"
        ...
        <p className="text-xs text-white/40">
          Revisit the Setup Wizard to change your LLM provider configuration,
          update API keys, or re-verify HTTPS certificates. Owner PIN
      ...
      <section className="card p-6 space-y-4" data-testid="https-trust-card">
        <header>
          ...
          <div className="text-white/60 text-sm">
            Manage HTTPS certificate trust for LAN devices
          </div>
        </header>
        ...
        {httpsStatus ? (
          <div className="space-y-3">
                ...
                onClick={async () => {
                  ...
                  try {
                    const res = await fetch(
                      buildServerUrl("/api/admin/https/ca"),
                    );
            ...
            <p className="text-xs text-white/40">
              Install this certificate on other devices (phones, tablets,
              laptops) to trust AKIOR's HTTPS on your LAN. See the{" "}
              <a href="/setup" className="text-cyan-400 underline">
                Setup Wizard
              </a>{" "}
      ...
      <section className="card p-6 space-y-4" data-testid="remote-access-card">
        <header>
          ...
          <div className="text-white/60 text-sm">
            Access AKIOR securely from outside your home network
          </div>
        </header>
        ...
        {remoteStatus ? (
          <div className="space-y-3">
            ...
            {remoteStatus.enabled && remoteStatus.tailscaleStatus && (
              <div className="bg-white/5 rounded-lg p-3 space-y-2">
                <div className="flex items-center gap-2 text-sm">
                  <span
                    className={
                      remoteStatus.tailscaleStatus.running
                        ? "text-emerald-400"
                        : "text-amber-400"
                    }
                  >
                ...
                {remoteStatus.tailscaleStatus.serveActive && (
                  <div className="text-xs text-emerald-400">
                    ✓ Tailscale Serve active on port{" "}
                    {remoteStatus.servePort || 3000}
                  </div>
                )}
              </div>
            )}
            ...
            <p className="text-xs text-white/40">
              ...
              to securely connect from anywhere. Configure in the{" "}
              <a href="/setup" className="text-cyan-400 underline">
                Setup Wizard
              </a>
              .
      ...
      <section className="card p-6 space-y-6">
        <header>
          ...
          <div className="text-white/60 text-sm">
            Choose your interface theme for AKIOR
          </div>
        </header>
      ...
      <section id="provider-keys" className="card p-6 space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2 rounded-xl border border-white/10 px-4 py-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-xs text-white/40">
                  Managed on the server. Update it from the Provider keys
                  section below.
                </p>
              </div>
          ...
          <label className="space-y-2">
            ...
            <p className="text-xs text-white/40">
              Choose the realtime model for voice conversations. Mini models are
              more cost-effective.
            </p>
          </label>
          <label className="space-y-2">
          ...
          <label className="space-y-2">
            ...
            <select
              ...
              onChange={(event) => {
                updateJarvisSettings({
                  imageDetail: event.target.value as "low" | "high" | "auto",
                });
                setSettings(readSettings());
              }}
            >
            ...
            <p className="text-xs text-white/40">
              Set image processing detail for camera vision. "Low" uses only 85
              tokens per image, significantly reducing rate limits.
            </p>
          </label>
          <label className="space-y-2 md:col-span-2">
            ...
            />
            <p className="text-xs text-white/40">
              Define AKIOR's personality, tone, and behavior. This is sent at
          ...
          <label className="space-y-2">
            ...
            />
            <p className="text-xs text-white/40">
              Say "Hey &lt;trigger&gt;" to start a session. Example: "Hey
              akior".
            </p>
          </label>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="space-y-2 md:col-span-2">
            ...
            <textarea
              ...
              onChange={(event) =>
                updateTextChat("initialPrompt", event.target.value)
              }
              placeholder="You are a thoughtful GPT-5 assistant…"
            />
            <p className="text-xs text-white/40">
              Sent as a developer message before the first user message to steer
              tone and behavior.
            </p>
          </label>
          <label className="space-y-2">
          ...
          <label className="space-y-2">
            ...
            <select
              ...
              onChange={(event) =>
                updateTextChat(
                  "reasoningEffort",
                  event.target.value as TextChatSettings["reasoningEffort"],
                )
              }
            >
          ...
          <label className="space-y-2">
            ...
            <select
              ...
              onChange={(event) =>
                updateTextChat(
                  "verbosity",
                  event.target.value as TextChatSettings["verbosity"],
                )
              }
            >
              <option className="bg-[#0b0f14]" value="low">
          ...
          <label className="flex items-center gap-3 md:col-span-2">
            <input
              ...
              onChange={(event) =>
                updateTextChat("useWebSearch", event.target.checked)
              }
            />
            <div>
              <div className="text-sm text-white/70">
                Allow web search for answers
              </div>
              <p className="text-xs text-white/40 mt-1">
                When enabled, searches the web to provide up-to-date
                information. Requires Web Search integration.
              </p>
            </div>
          </label>
          <label className="flex items-center gap-3 md:col-span-2">
            <input
              ...
              onChange={(event) =>
                updateTextChat("useLocalLlm", event.target.checked)
              }
            />
            <div>
              <div className="text-sm text-white/70">
              ...
              <p className="text-xs text-white/40 mt-1">
                When enabled and Local LLM is connected, AKIOR will use the
                local model for text chat (with optional web search
                augmentation).
              </p>
            </div>
          </label>
          ...
          <label className="space-y-2 md:col-span-2">
            ...
            <select
              ...
              onChange={(event) =>
                updateTextChat("ttsProvider", event.target.value as any)
              }
            >
            ...
            <p className="text-xs text-white/40">
              Choose which TTS service to use for "Speak answer" in Chat.
              Provider must be configured in Integrations.
            </p>
          </label>
        </div>
      </section>
      ...
      <section className="card p-6 space-y-4">
        <header>
          ...
          <div className="text-white/60 text-sm">
            Manage API credentials stored on the server
          </div>
        </header>
        <div className="space-y-6">
          {renderKeyRow(
            "openai",
            "OpenAI API key",
            "Used for AKIOR realtime voice sessions.",
          )}
          {renderKeyRow(
            "meshy",
            "Meshy API key",
            "Required for Meshy 3D model generation.",
          )}
        </div>
        <p className="text-xs text-white/40">
          Keys are saved on the server and never sent back to browsers. Entering
          a new value replaces the stored key.
        </p>
      </section>
      ...
      <section className="card p-6 space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="space-y-2">
            ...
            <select
              ...
              onChange={(event) =>
                updateModel("topology", event.target.value as any)
              }
            >
          ...
          <label className="space-y-2">
            ...
            />
            <p className="text-xs text-white/40">
              Adjust per plan limits. Default 30k polygons.
            </p>
          </label>
          <label className="space-y-2">
            ...
            <select
              ...
              onChange={(event) =>
                updateModel("symmetryMode", event.target.value as any)
              }
            >
              <option className="bg-[#0b0f14]" value="auto">
                Auto
          ...
          <label className="space-y-2">
            <div className="text-sm text-white/70">
              Preferred art style (Text to 3D)
            </div>
            <select
              className="w-full bg-transparent border border-white/10 rounded-xl px-3 py-2"
              value={(modelValue("artStyle") as string) ?? "realistic"}
              onChange={(event) =>
                updateModel("artStyle", event.target.value as any)
              }
            >
          ...
          <label className="space-y-2">
            ...
            <select
              ...
              onChange={(event) =>
                updateModel("outputFormat", event.target.value as any)
              }
            >
            ...
            <p className="text-xs text-white/40">
              Only this format will be downloaded. Export STL from viewer.
            </p>
          </label>
        </div>
        <div className="grid gap-3 md:grid-cols-3">
          <label className="flex items-center gap-3 text-sm">
            <input
              ...
              onChange={(event) =>
                updateModel("shouldRemesh", event.target.checked)
              }
            />
            Enable remesh
          </label>
          <label className="flex items-center gap-3 text-sm">
            <input
              ...
              onChange={(event) =>
                updateModel("shouldTexture", event.target.checked)
              }
            />
            Generate textures
          </label>
          <label className="flex items-center gap-3 text-sm">
            <input
              ...
              onChange={(event) =>
                updateModel("enablePbr", event.target.checked)
              }
            />
            Enable PBR maps
          </label>
        ...
        <p className="text-xs text-white/40">
          These defaults are applied to Capture, Upload, and Text prompt
          workflows. You can override them per task from the creation page.
        </p>
      </section>
      ...
      <section className="card p-6 space-y-4">
        <header className="flex flex-col gap-1">
          ...
          <div className="text-white/60 text-sm">
            Login with your Bambu Labs credentials to monitor and control your
            3D printers
          </div>
        </header>
        ...
        {!showVerifyForm ? (
          <div className="space-y-4">
            ...
            <label className="space-y-2">
              ...
              />
              <p className="text-xs text-white/40">
                Enter your Bambu Labs account password. You'll receive a
                verification code via email if needed.
              </p>
            </label>
          ...
          <div className="space-y-4">
            <label className="space-y-2">
              ...
              />
              <p className="text-xs text-white/40">
                Enter the code sent to {bambuEmail}
              </p>
            </label>
            <button
      ...
      <section className="card p-6 space-y-6">
        <header>
          ...
          <div className="text-white/60 text-sm">
            Manage external services and providers
          </div>
        </header>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">
                  {integrationMetadata.webSearch.name}
                </div>
                <div className="text-xs text-white/50 mt-1">
                  {integrationMetadata.webSearch.description}
                </div>
              </div>
              <div
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={!!settings?.integrations?.webSearch?.enabled}
                onChange={(e) => {
                  updateIntegration("webSearch", { enabled: e.target.checked });
                  setSettings(readSettings());
                }}
              />
              Enable
              ...
              <div className="space-y-3 pt-2 border-t border-white/10">
                ...
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent border border-white/10 rounded px-2 py-1 text-sm"
                    value={
                      settings?.integrations?.webSearch?.defaultRegion ?? ""
                    }
                    onChange={(e) => {
                      updateIntegration("webSearch", {
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">
                  {integrationMetadata.localLLM.name}
                </div>
                <div className="text-xs text-white/50 mt-1">
                  {integrationMetadata.localLLM.description}
                </div>
              </div>
              <div
                className={`px-2 py-1 rounded text-xs ${
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={!!settings?.integrations?.localLLM?.enabled}
                onChange={(e) => {
                  updateIntegration("localLLM", { enabled: e.target.checked });
                  setSettings(readSettings());
                }}
              />
              Enable
              ...
              <div className="space-y-3 pt-2 border-t border-white/10">
                <label className="space-y-1">
                  ...
                  <select
                    className="w-full bg-transparent border border-white/10 rounded px-2 py-1 text-sm"
                    value={
                      settings?.integrations?.localLLM?.provider ?? "ollama"
                    }
                    onChange={(e) => {
                      updateIntegration("localLLM", {
                        provider: e.target.value as "ollama" | "custom-http",
                      });
                      setSettings(readSettings());
                    }}
                  >
                    <option className="bg-[#0b0f14]" value="ollama">
                      Ollama (local HTTP)
                    </option>
                    <option className="bg-[#0b0f14]" value="custom-http">
                      Custom HTTP API
                    </option>
                  </select>
                </label>
                ...
                <label className="space-y-1">
                  ...
                  ollamaModels.length > 0 ? (
                    ...
                    >
                      <option className="bg-[#0b0f14]" value="">
                        Select a model...
                      </option>
                      {ollamaModels.map((m) => (
                        <option
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">
                  {integrationMetadata.elevenLabs.name}
                </div>
                <div className="text-xs text-white/50 mt-1">
                  {integrationMetadata.elevenLabs.description}
                </div>
              </div>
              <div
                className={`px-2 py-1 rounded text-xs ${
                  ...
                  <div className="space-y-3 pt-2">
                    <label className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full bg-transparent border border-white/10 rounded px-2 py-1 text-sm"
                        value={
                          settings?.integrations?.elevenLabs?.stability ?? ""
                        }
                        onChange={(e) => {
                          updateIntegration("elevenLabs", {
                    ...
                    <label className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full bg-transparent border border-white/10 rounded px-2 py-1 text-sm"
                        value={
                          settings?.integrations?.elevenLabs?.similarityBoost ??
                          ""
                        }
                        onChange={(e) => {
                          updateIntegration("elevenLabs", {
                    ...
                    <label className="space-y-1">
                      ...
                      />
                      <div className="text-xs text-white/40">
                        Default: 0.0 (only supported by some models)
                      </div>
                    </label>
                  </div>
                </details>
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">
                  {integrationMetadata.azureTTS.name}
                </div>
                <div className="text-xs text-white/50 mt-1">
                  {integrationMetadata.azureTTS.description}
                </div>
              </div>
              <div
                className={`px-2 py-1 rounded text-xs ${
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={!!settings?.integrations?.azureTTS?.enabled}
                onChange={(e) => {
                  updateIntegration("azureTTS", { enabled: e.target.checked });
                  setSettings(readSettings());
                }}
              />
              Enable
              ...
              <div className="space-y-3 pt-2 border-t border-white/10">
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      updateIntegration("azureTTS", { region: e.target.value });
                      setSettings(readSettings());
                    }}
                    placeholder="eastus"
                  />
                  <div className="text-xs text-white/40">
                    e.g. eastus, westus, westeurope
                  </div>
                </label>
                <label className="space-y-1">
                  <div className="text-xs text-white/60">API Key</div>
                  <input
                    ...
                    onChange={(e) => {
                      updateIntegration("azureTTS", { apiKey: e.target.value });
                      setSettings(readSettings());
                    }}
                    placeholder="********"
                  />
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">
                  {integrationMetadata.spotify.name}
                </div>
                <div className="text-xs text-white/50 mt-1">
                  {integrationMetadata.spotify.description}
                </div>
              </div>
              <div
                className={`px-2 py-1 rounded text-xs ${
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(e) => {
                  updateIntegration("spotify", { enabled: e.target.checked });
                  setSettings(readSettings());
                }}
              />
              Enable
              ...
              <div className="space-y-3 pt-2 border-t border-white/10">
                ...
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      updateIntegration("spotify", {
                        defaultMarket: e.target.value || null,
                      });
                      setSettings(readSettings());
                    }}
                    placeholder="US"
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">
                  {integrationMetadata.gmail.name}
                </div>
                <div className="text-xs text-white/50 mt-1">
                  {integrationMetadata.gmail.description}
                </div>
              </div>
              <div
                className={`px-2 py-1 rounded text-xs ${
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={!!settings?.integrations?.gmail?.enabled}
                onChange={(e) => {
                  updateIntegration("gmail", { enabled: e.target.checked });
                  setSettings(readSettings());
                }}
              />
              Enable
              ...
              <div className="space-y-3 pt-2 border-t border-white/10">
                ...
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      updateIntegration("gmail", {
                        redirectUri: e.target.value || null,
                      });
                      setSettings(readSettings());
                    }}
                  ...
                  />
                  <div className="text-xs text-white/40">
                    Where Google redirects after OAuth consent
                  </div>
                </label>
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      updateIntegration("gmail", { userEmail: e.target.value });
                      setSettings(readSettings());
                    }}
                ...
                <label className="space-y-1">
                  ...
                  />
                  <div className="text-xs text-white/40">
                    Obtain via manual OAuth2 consent flow (see docs)
                  </div>
                </label>
              </div>
            )}
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">
                  {integrationMetadata.googleCalendar.name}
                </div>
                <div className="text-xs text-white/50 mt-1">
                  {integrationMetadata.googleCalendar.description}
                </div>
              </div>
              <div
                className={`px-2 py-1 rounded text-xs ${
              ...
              <div className="space-y-3 pt-2 border-t border-white/10">
                ...
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent border border-white/10 rounded px-2 py-1 text-sm"
                    value={
                      settings?.integrations?.googleCalendar?.redirectUri ?? ""
                    }
                    onChange={(e) => {
                      updateIntegration("googleCalendar", {
                        redirectUri: e.target.value || null,
                  ...
                  />
                  <div className="text-xs text-white/40">
                    Used during OAuth consent flow
                  </div>
                </label>
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent border border-white/10 rounded px-2 py-1 text-sm"
                    value={
                      settings?.integrations?.googleCalendar?.calendarId ?? ""
                    }
                    onChange={(e) => {
                      updateIntegration("googleCalendar", {
                  ...
                  />
                  <div className="text-xs text-white/40">
                    Use 'primary' for your main calendar or a specific calendar
                    ID
                  </div>
                </label>
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent border border-white/10 rounded px-2 py-1 text-sm"
                    value={
                      settings?.integrations?.googleCalendar?.refreshToken ?? ""
                    }
                    onChange={(e) => {
                      updateIntegration("googleCalendar", {
                        refreshToken: e.target.value,
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">
                  {integrationMetadata.alexa.name}
                </div>
                <div className="text-xs text-white/50 mt-1">
                  {integrationMetadata.alexa.description}
                </div>
              </div>
              <div
                className={`px-2 py-1 rounded text-xs ${
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={!!settings?.integrations?.alexa?.enabled}
                onChange={(e) => {
                  updateIntegration("alexa", { enabled: e.target.checked });
                  setSettings(readSettings());
                }}
              />
              Enable
              ...
              <div className="space-y-3 pt-2 border-t border-white/10">
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      updateIntegration("alexa", { clientId: e.target.value });
                      setSettings(readSettings());
                    }}
                ...
                <label className="space-y-1">
                  ...
                  />
                  <div className="text-xs text-white/40">
                    Obtain via Amazon's Login with Amazon OAuth flow
                  </div>
                </label>
                <label className="space-y-1">
                  <div className="text-xs text-white/60">Region</div>
                  <select
                    ...
                    onChange={(e) => {
                      updateIntegration("alexa", { region: e.target.value });
                      setSettings(readSettings());
                    }}
                  >
                  ...
                  <div className="text-xs text-white/40">
                    Select your Alexa device region
                  </div>
                </label>
                <button
                  ...
                  disabled={
                    !settings?.integrations?.alexa?.clientId ||
                    !settings?.integrations?.alexa?.clientSecret
                  }
                >
                  Test Connection
                </button>
                {smarthomeTestResults.alexa && (
                  <p
                    className={`text-xs ${
                      smarthomeTestResults.alexa.type === "success"
                        ? "text-emerald-400"
                        : "text-red-400"
                    }`}
                  >
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">
                  {integrationMetadata.irobot.name}
                </div>
                <div className="text-xs text-white/50 mt-1">
                  {integrationMetadata.irobot.description}
                </div>
              </div>
              <div
                className={`px-2 py-1 rounded text-xs ${
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={!!settings?.integrations?.irobot?.enabled}
                onChange={(e) => {
                  updateIntegration("irobot", { enabled: e.target.checked });
                  setSettings(readSettings());
                }}
              />
              Enable
              ...
              <div className="space-y-3 pt-2 border-t border-white/10">
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      updateIntegration("irobot", { username: e.target.value });
                      setSettings(readSettings());
                    }}
                    placeholder="your-email@example.com"
                  />
                  <div className="text-xs text-white/40">
                    Your iRobot account email
                  </div>
                </label>
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      updateIntegration("irobot", { password: e.target.value });
                      setSettings(readSettings());
                    }}
                    placeholder="********"
                  />
                  <div className="text-xs text-white/40">
                    Your iRobot account password
                  </div>
                </label>
                <label className="space-y-1">
                  <div className="text-xs text-white/60">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      updateIntegration("irobot", {
                        robotId: e.target.value || undefined,
                      });
                      setSettings(readSettings());
                    }}
                  ...
                  />
                  <div className="text-xs text-white/40">
                    Leave empty to auto-detect or specify a specific robot
                  </div>
                </label>
                <button
                  className="btn w-full"
                  ...
                  disabled={
                    !settings?.integrations?.irobot?.username ||
                    !settings?.integrations?.irobot?.password
                  }
                >
                  Test Connection
                </button>
                {smarthomeTestResults.irobot && (
                  <p
                    className={`text-xs ${
                      smarthomeTestResults.irobot.type === "success"
                        ? "text-emerald-400"
                        : "text-red-400"
                    }`}
                  >
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">
                  {integrationMetadata.nest.name}
                </div>
                <div className="text-xs text-white/50 mt-1">
                  {integrationMetadata.nest.description}
                </div>
              </div>
              <div
                className={`px-2 py-1 rounded text-xs ${
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={!!settings?.integrations?.nest?.enabled}
                onChange={(e) => {
                  updateIntegration("nest", { enabled: e.target.checked });
                  setSettings(readSettings());
                }}
              />
              Enable
              ...
              <div className="space-y-3 pt-2 border-t border-white/10">
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      updateIntegration("nest", { projectId: e.target.value });
                      setSettings(readSettings());
                    }}
                ...
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      updateIntegration("nest", { clientId: e.target.value });
                      setSettings(readSettings());
                    }}
                ...
                <label className="space-y-1">
                  ...
                  />
                  <div className="text-xs text-white/40">
                    Obtain via Google OAuth2 consent flow
                  </div>
                </label>
                <label className="space-y-1">
                  <div className="text-xs text-white/60">
                  ...
                  />
                  <div className="text-xs text-white/40">
                    Leave empty to auto-detect or specify a specific thermostat
                  </div>
                </label>
                <button
                  className="btn w-full"
                  ...
                  disabled={
                    !settings?.integrations?.nest?.projectId ||
                    !settings?.integrations?.nest?.clientId
                  }
                >
                  Test Connection
                </button>
                {smarthomeTestResults.nest && (
                  <p
                    className={`text-xs ${
                      smarthomeTestResults.nest.type === "success"
                        ? "text-emerald-400"
                        : "text-red-400"
                    }`}
                  >
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">
                  {integrationMetadata.smartLights.name}
                </div>
                <div className="text-xs text-white/50 mt-1">
                  {integrationMetadata.smartLights.description}
                </div>
              </div>
              <div
                className={`px-2 py-1 rounded text-xs ${
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={!!settings?.integrations?.smartLights?.enabled}
                onChange={(e) => {
                  updateIntegration("smartLights", {
                    enabled: e.target.checked,
                  });
                  setSettings(readSettings());
                }}
              ...
              <div className="space-y-3 pt-2 border-t border-white/10">
                <label className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      updateIntegration("smartLights", {
                        provider: e.target.value as "hue" | "lifx",
                      });
                      setSettings(readSettings());
                    }}
                  >
                  ...
                  <div className="text-xs text-white/40">
                    Select your smart light provider
                  </div>
                </label>
                <label className="space-y-1">
                  <div className="text-xs text-white/60">API Key</div>
                ...
                {settings?.integrations?.smartLights?.provider === "hue" && (
                  <label className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-transparent border border-white/10 rounded px-2 py-1 text-sm"
                      value={
                        settings?.integrations?.smartLights?.bridgeIp ?? ""
                      }
                      onChange={(e) => {
                        updateIntegration("smartLights", {
                          bridgeIp: e.target.value || undefined,
                        });
                        setSettings(readSettings());
                      }}
                ...
                {smarthomeTestResults.smartLights && (
                  <p
                    className={`text-xs ${
                      smarthomeTestResults.smartLights.type === "success"
                        ? "text-emerald-400"
                        : "text-red-400"
                    }`}
                  >
      ...
      <section className="card p-6 space-y-6">
        <header>
          ...
          <div className="text-white/60 text-sm">
            Configure cameras, motion detection, and lockdown mode
          </div>
        </header>
        ...
        <div className="border border-white/10 rounded-xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-white/50 mt-1">
                Secure all doors, arm alarms, and lock down cameras
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div
          ...
          {settings?.lockdownState?.active && (
            <div className="pt-3 border-t border-white/10 grid grid-cols-3 gap-3 text-xs">
              <div className="flex items-center gap-2">
                <span
                  className={`h-2 w-2 rounded-full ${
                    settings.lockdownState.features.doorsLocked
                      ? "bg-red-400"
                      : "bg-white/20"
                  }`}
                />
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`h-2 w-2 rounded-full ${
                    settings.lockdownState.features.alarmArmed
                      ? "bg-red-400"
                      : "bg-white/20"
                  }`}
                />
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`h-2 w-2 rounded-full ${
                    settings.lockdownState.features.camerasSecured
                      ? "bg-red-400"
                      : "bg-white/20"
                  }`}
                />
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button className="btn px-3 py-1 text-xs" onClick={handleAddCamera}>
              + Add Camera
            </button>
          </div>
          ...
          {settings?.cameras && settings.cameras.length > 0 ? (
                          ...
                          <>
                            ...
                            <label className="space-y-1">
                              <div className="flex items-center justify-between text-xs">
                                ...
                                <span className="text-white/80">
                                  {camera.motionDetection.sensitivity}
                                </span>
                              </div>
                              <input
                                type="range"
                            ...
                            <div className="pt-2 border-t border-white/10">
                              ...
                              <div className="bg-white/5 border border-white/10 rounded p-3 text-center">
                                <div className="text-xs text-white/40">
                                  📐 Zone-based motion detection will be
                                  available in a future update. Configure
                                  specific areas of the camera view to monitor.
                                </div>
                              </div>
                            </div>
                          </>
            ...
            <div className="border border-white/10 rounded-xl p-6 text-center text-sm text-white/60">
              No cameras configured. Click "Add Camera" to register a new camera
              device.
            </div>
          )}
        </div>
      ...
      <section className="card p-6 space-y-6">
        <header>
          ...
          <div className="text-white/60 text-sm">
            View conversation history, action timeline, and system logs
          </div>
        </header>
          ...
          {activeMemoryTab === "actions" && (
            <div>
              <div className="mb-4">
                ...
                <p className="text-sm text-white/50">
                  View a chronological timeline of all actions performed by
                  AKIOR and user interactions.
                </p>
              </div>
              <ActionTimeline />
            </div>
          ...
          {activeMemoryTab === "logs" && (
            <div>
              <div className="mb-4">
                ...
                <p className="text-sm text-white/50">
                  Access system logs for debugging and monitoring application
                  events.
                </p>
              </div>
              <LogViewer />
      ...
      <section className="card p-6 space-y-6">
        <header>
          <div className="text-lg font-semibold">Avatar Selection</div>
          <div className="text-white/60 text-sm">
            Choose a VRM avatar for VCam virtual camera
          </div>
        </header>

        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm text-white/70">VCam Virtual Camera</div>
            <div className="text-xs text-white/40">
              Enable VCam to use your avatar as a virtual webcam
            </div>
          </div>
          <button
            type="button"
            onClick={() => {
              updateAvatarSettings({
                vcamEnabled: !settings?.avatar?.vcamEnabled,
              });
              setSettings(readSettings());
            }}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${settings?.avatar?.vcamEnabled ? "bg-blue-500" : "bg-white/20"}`}
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${settings?.avatar?.vcamEnabled ? "translate-x-6" : "translate-x-1"}`}
            />
          </button>
        </div>

        <div>
          <div className="text-sm text-white/70 mb-3">Available Avatars</div>
          <div className="text-xs text-white/40 mb-3">
            VRM files from ~/akior/avatars/
          </div>
          <div className="grid grid-cols-3 gap-4">
            {(
              [
                {
                  file: "orion.vrm",
                  name: "Orion",
                  desc: "CC0 sci-fi character",
                  icon: "\u{1F9D1}\u200D\u{1F680}",
                },
                {
                  file: "aurora.vrm",
                  name: "Aurora",
                  desc: "CC0 stylized avatar",
                  icon: "\u2728",
                },
                {
                  file: "devil.vrm",
                  name: "Devil",
                  desc: "CC0 dark themed avatar",
                  icon: "\u{1F608}",
                },
              ] as const
            ).map((avatar) => (
              <button
                ...
                type="button"
                onClick={() => {
                  updateAvatarSettings({ selectedAvatar: avatar.file });
                  setSettings(readSettings());
                }}
                className={`relative p-4 rounded-xl border-2 transition-all text-left ${settings?.avatar?.selectedAvatar === avatar.file ? "border-blue-500 bg-blue-500/10 shadow-lg shadow-blue-500/20" : "border-white/10 bg-white/5 hover:border-white/30 hover:bg-white/10"}`}
              >
                <div className="w-full aspect-square rounded-lg bg-gradient-to-br from-white/10 to-white/5 mb-3 flex items-center justify-center text-3xl">
                  {avatar.icon}
                </div>
                <div className="font-medium text-sm">{avatar.name}</div>
                <div className="text-xs text-white/50 mt-1">{avatar.desc}</div>
                <div className="text-xs text-white/30 mt-1 font-mono">
                  {avatar.file}
                </div>
                {settings?.avatar?.selectedAvatar === avatar.file && (
        ...
        {settings?.avatar?.selectedAvatar && (
          <div className="p-4 rounded-lg bg-white/5 border border-white/10">
            <div className="text-sm text-white/70 mb-2">Active Avatar</div>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-blue-500/20 flex items-center justify-center text-lg">
                {settings.avatar.selectedAvatar === "orion.vrm"
                  ? "\u{1F9D1}\u200D\u{1F680}"
                  : settings.avatar.selectedAvatar === "aurora.vrm"
                    ? "\u2728"
                    : "\u{1F608}"}
              </div>
              <div>
                <div className="text-sm font-medium">
                  {settings.avatar.selectedAvatar.replace(".vrm", "")}
                </div>
                <div className="text-xs text-white/40 font-mono">
                  ~/akior/avatars/{settings.avatar.selectedAvatar}
                </div>
              </div>
            </div>
          </div>
      ...
      <footer className="text-center text-xs text-white/40 pt-4 space-y-1">
        <BuildInfo />
        <a href="/diagnostics" className="hover:text-white/60 underline">
          View Diagnostics
        </a>
      </footer>
    </div>
