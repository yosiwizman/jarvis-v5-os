...
const SETUP_STEPS = [
  {
    id: "pin",
    title: "Set Owner PIN",
    description: "Secure admin access to settings",
  },
  {
    id: "https",
    title: "Trust HTTPS Certificate",
    description: "Enable secure mic/camera access",
  },
  {
    id: "remote",
    title: "Remote Access",
    description: "Optional - access AKIOR from anywhere",
  },
  {
    id: "llm",
    title: "Configure LLM Provider",
    description: "Required for voice assistant and AI features",
  },
  {
    id: "meshy",
    title: "Configure Meshy API Key",
    description: "Optional - enables 3D model generation",
  },
] as const;
...
function SecureContextStatus() {
  ...
  return (
    <div
      className={`rounded-xl p-4 ${isSecure ? "bg-emerald-500/10 border border-emerald-500/30" : "bg-amber-500/10 border border-amber-500/30"}`}
    >
      ...
      {!isSecure && (
        <div className="mt-3 p-3 bg-black/30 rounded-lg">
          <p className="text-xs text-white/60 mb-2">
            On Windows, run this command to install the certificate:
          </p>
...
export default function SetupPage() {
  ...
  const [validation, setValidation] = useState<ValidationState>({
    openai: { validating: false },
    meshy: { validating: false },
  });
  const [feedback, setFeedback] = useState<
    Record<KeyName, { type: "success" | "error"; message: string } | undefined>
  >({
    openai: undefined,
    meshy: undefined,
  });
  ...
  // Client-side hydration-safe state for isSecureContext
  const [isSecureCtx, setIsSecureCtx] = useState<boolean | null>(null);
  useEffect(() => {
  ...
  const refreshHttpsStatus = useCallback(async () => {
    try {
      const response = await fetch(buildServerUrl("/admin/https/status"), {
        credentials: "include",
      });
      if (response.ok) {
        const payload = await response.json();
  ...
  const refreshRemoteStatus = useCallback(async () => {
    try {
      const response = await fetch(
        buildServerUrl("/admin/remote-access/status"),
        { credentials: "include" },
      );
      if (response.ok) {
        const payload = await response.json();
  ...
  const handleDownloadCert = async () => {
    ...
    try {
      const response = await fetch(buildServerUrl("/admin/https/ca"), {
        credentials: "include",
      });
      if (response.ok) {
  ...
  const handleEnableRemote = async () => {
    ...
    try {
      ...
      const response = await apiFetch(
        buildServerUrl("/admin/remote-access/enable"),
        {
          method: "POST",
      ...
      if (result.ok) {
        ...
        setRemoteFeedback({
          type: "error",
          message: result.message || "Failed to enable remote access",
        });
      }
    } catch (error) {
      const csrfMsg =
      ...
      setRemoteFeedback({
        type: "error",
        message: csrfMsg || "Failed to enable remote access",
      });
    } finally {
  ...
  const handleDisableRemote = async () => {
    ...
    try {
      const response = await apiFetch(
        buildServerUrl("/admin/remote-access/disable"),
        {
          method: "POST",
        },
      );
      ...
      if (result.ok) {
        ...
        setRemoteFeedback({
          type: "error",
          message: result.message || "Failed to disable remote access",
        });
      }
    } catch (error) {
      const csrfMsg =
      ...
      setRemoteFeedback({
        type: "error",
        message: csrfMsg || "Failed to disable remote access",
      });
    } finally {
  ...
  const handleLLMTest = async () => {
    ...
    try {
      ...
      setLlmTestResult({
        ok: false,
        error: csrfMsg || "Failed to test connection",
      });
    } finally {
      setLlmTesting(false);
    }
  ...
  const handleLLMSave = async () => {
    ...
    try {
      ...
      setLlmFeedback({
        type: "error",
        message: csrfMsg || "Failed to save configuration",
      });
    } finally {
      setLlmSaving(false);
    }
  ...
  const validateKey = async (name: KeyName) => {
    ...
    try {
      ...
      setValidation((prev) => ({
        ...
        [name]: {
          validating: false,
          result: { ok: false, error: csrfMsg || "Validation failed" },
        },
      }));
    }
  };
  ...
  const saveKey = async (name: KeyName) => {
    ...
    try {
      ...
      setFeedback((prev) => ({
        ...prev,
        [name]: { type: "success", message: "Key saved successfully!" },
      }));
    } catch (error) {
      const csrfMsg =
        error instanceof CsrfError
  ...
  const getStepStatus = (
    ...
    if (stepId === "https") {
      if (isSecureCtx) return "complete";
    ...
    <div className="max-w-3xl mx-auto space-y-8" data-testid="setup-page">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-white/60">
          Complete these steps to get started with your AI assistant
        </p>
      ...
      {allComplete && (
        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-6 text-center">
          ...
          <p className="text-white/70 mb-4">
            Your {BRAND.productName} system is ready to use.
          </p>
          <Link
            href="/menu"
          ...
          return (
            ...
            >
              ...
              <div className="flex-1">
                <div
                  className={`font-medium ${stepStatus === "upcoming" ? "text-white/50" : ""}`}
                >
                  {step.title}
                </div>
        ...
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-white/60 text-sm">
              Secure admin access with a 4-8 digit PIN
            </p>
          </div>
          {pinConfigured && (
        ...
        {pinConfigured ? (
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-white/40">
              This PIN protects admin access to Setup and Settings. Store it
              securely - there is no recovery option.
            </p>
          </div>
        )}
      </section>
      ...
      <section id="https" className="card p-6 space-y-4">
          ...
          <div className="bg-black/30 rounded-lg p-3 text-xs">
            {certTab === "windows" && (
              <div className="space-y-2">
                ...
                <code className="block text-cyan-400 font-mono">
                  .\ops\trust-lan-https.ps1 -Apply
                </code>
            ...
            {certTab === "macos" && (
              <div className="space-y-2">
                ...
                <p className="text-white/60">
                  2. Open Keychain Access → System → Certificates
                </p>
                <p className="text-white/60">
                  3. Drag the .pem file into the list
                </p>
                <p className="text-white/60">
                  4. Double-click it → Trust → "Always Trust"
                </p>
            ...
            {certTab === "ios" && (
              <div className="space-y-2">
                <p className="text-white/60">
                  1. Download cert on device (or AirDrop from Mac)
                </p>
                <p className="text-white/60">
                ...
                <p className="text-white/60">
                  4. Enable trust for "Caddy Local Authority"
                </p>
              </div>
            )}
            {certTab === "android" && (
              <div className="space-y-2">
                ...
                <p className="text-white/60">
                  2. Settings → Security → Encryption → Install from storage
                </p>
                <p className="text-white/60">
                  3. Select the downloaded .pem file
                </p>
                <p className="text-white/60">
                  4. Name it "AKIOR LAN CA" and select "VPN and apps"
                </p>
                <p className="text-white/40">
                  Location varies by Android version/vendor.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="text-xs text-amber-400/80 bg-amber-500/10 rounded-lg p-2">
          💡 Prefer <code className="font-mono">akior.home.arpa</code> over{" "}
          <code className="font-mono">akior.local</code> for reliable
          resolution.
        </div>
      ...
      <section id="remote-access" className="card p-6 space-y-4">
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-white/60 text-sm">
              Access {BRAND.productName} from anywhere via Tailscale
            </p>
          </div>
        ...
        >
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            <div
              className={
                remoteStatus?.tailscaleInstalled
                  ? "text-emerald-400"
                  : "text-amber-400"
              }
            >
              {remoteStatus?.tailscaleInstalled
                ? remoteStatus.tailscaleUp
                  ? "✓ Connected"
                  : "⚠ Not connected"
                : "✗ Not installed"}
            </div>
            ...
            {remoteStatus?.suggestedUrl && (
              <>
                ...
                <a
                  href={remoteStatus.suggestedUrl}
                  target="_blank"
          ...
          <div className="space-y-3">
            <div>
              <label className="block text-sm text-white/60 mb-2">
                Auth Key{" "}
                <span className="text-white/30">(one-time, optional)</span>
              </label>
              <input
                type="password"
              ...
              <p className="text-xs text-amber-400/70 mt-1">
                ⚠ Not stored. Only needed if Tailscale is not already connected.
              </p>
            </div>
            <button
              className="btn"
      ...
      <section className="card p-6 space-y-4">
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-white/60 text-sm">
              Required for voice assistant and AI features
            </p>
          </div>
          {llmConfig?.keyConfigured && (
            <span className="text-xs px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded-full">
          ...
          {llmConfig?.keyConfigured && (
            <div className="text-xs text-white/40 pt-2 border-t border-white/5">
              ...
              <span className="text-white/60">
                {llmConfig.provider === "openai-cloud"
                  ? "OpenAI Cloud"
                  : "Local/Compatible"}
              </span>
              {llmConfig.baseUrlHost && (
                <>
                  {" "}
                  at{" "}
                  <span className="text-white/60">{llmConfig.baseUrlHost}</span>
                </>
              )}
            </div>
          )}
      ...
      <section className="card p-6 space-y-4">
        ...
        <div className="space-y-3">
          <input
            ...
            onChange={(e) =>
              setPendingKeys((prev) => ({ ...prev, meshy: e.target.value }))
            }
            placeholder="msy_..."
          ...
          {feedback.meshy && (
            <div
              className={`text-sm ${feedback.meshy.type === "error" ? "text-red-400" : "text-emerald-400"}`}
            >
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={
                !pendingKeys.meshy.trim() ||
                validation.meshy.validating ||
                savingKeys.meshy
              }
            >
