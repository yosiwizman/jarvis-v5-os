...
/**
 * Google Drive Subpage Smoke Tests
 *
 * Covers the /settings/channels/drive route backed by the DEC-033
...
test.describe('Google Drive Subpage Smoke', () => {
  ...
  test('Drive subpage testid is visible and Google Drive provider card is visible', async ({ page }) => {
    await page.goto('/settings/channels/drive', { waitUntil: 'domcontentloaded' });

    // The root div must carry the expected testid
    await expect(
      page.getByTestId('channels-drive-subpage')
