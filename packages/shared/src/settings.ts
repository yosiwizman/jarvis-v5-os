...
export type AvatarSettings = {
  selectedAvatar?: string; // filename of the selected VRM avatar
  avatarsDir?: string; // directory where VRM files are stored
  vcamEnabled?: boolean; // whether VCam virtual camera is active
};
...
function normalizeIntegrations(
  input: Partial<IntegrationSettings> | undefined | null,
): IntegrationSettings {
  ...
  return {
    weather: {
      ...defaultIntegrationSettings.weather,
      ...(input.weather ?? {}),
    },
    webSearch: {
      ...defaultIntegrationSettings.webSearch,
      ...(input.webSearch ?? {}),
    },
    localLLM: {
      ...defaultIntegrationSettings.localLLM,
      ...(input.localLLM ?? {}),
    },
    elevenLabs: {
      ...defaultIntegrationSettings.elevenLabs,
      ...(input.elevenLabs ?? {}),
    },
    azureTTS: {
      ...defaultIntegrationSettings.azureTTS,
      ...(input.azureTTS ?? {}),
    },
    spotify: {
      ...defaultIntegrationSettings.spotify,
      ...(input.spotify ?? {}),
    },
    gmail: { ...defaultIntegrationSettings.gmail, ...(input.gmail ?? {}) },
    googleCalendar: {
      ...defaultIntegrationSettings.googleCalendar,
    ...
    smartLights: {
      ...defaultIntegrationSettings.smartLights,
      ...(input.smartLights ?? {}),
    },
  };
}
...
export function normalizeSettings(input: unknown): AppSettings {
  ...
  const normalized: AppSettings = {
    ...
    avatar: { ...defaultSettings.avatar, ...(partial.avatar ?? {}) },
...
function assertNormalizedSettings(settings: AppSettings): void {
  ...
  if (!settings.integrations || typeof settings.integrations !== "object") {
    console.error(
      "[Settings] ASSERTION FAILED: integrations is missing or invalid",
      settings,
    );
    throw new Error(
      "[Settings] normalizeSettings failed: integrations is missing or invalid",
    );
  }
  ...
  for (const key of requiredIntegrationKeys) {
    if (
      ...
      typeof settings.integrations[key] !== "object"
    ) {
      console.error(
        `[Settings] ASSERTION FAILED: integrations.${key} is missing or invalid`,
        {
          key,
      ...
      throw new Error(
        `[Settings] normalizeSettings failed: integrations.${key} is missing or invalid`,
      );
    }
  }
}
...
const defaultSettings: AppSettings = {
  ...
  avatar: {
    selectedAvatar: "",
    avatarsDir: "/Users/yosiwizman/akior/avatars",
    vcamEnabled: false,
  },
  notificationPreferences: {
...
export async function loadSettingsFromServer(): Promise<AppSettings> {
  ...
  try {
    ...
    console.warn(
      "⚠️ Failed to load settings from server, using localStorage fallback:",
      error,
    );
...
export function updateImageGenerationSettings(
  partial: Partial<ImageGenerationSettings>,
) {
  const current = readSettings();
  writeSettings({
    ...current,
...
export function updateIntegrations(
  partial: Partial<import("./integrations.js").IntegrationSettings>,
) {
  const current = readSettings();
  writeSettings({
    ...current,
    integrations: { ...current.integrations, ...partial },
  });
}

export function updateIntegration<
  K extends import("./integrations.js").IntegrationId,
>(id: K, patch: Partial<import("./integrations.js").IntegrationSettings[K]>) {
  const current = readSettings();
  const updated = {
    ...current,
    integrations: {
...
export function updateCamera(
  cameraId: string,
  patch: Partial<import("./types.js").CameraSettings>,
) {
  const current = readSettings();
  ...
  if (existingIndex >= 0) {
    ...
    cameras.push({
      ...
      enabled: true,
      friendlyName: "",
      motionDetection: { enabled: false, sensitivity: 50, cooldownSeconds: 30 },
      ...patch,
    } as import("./types.js").CameraSettings);
  }

  writeSettings({ ...current, cameras });
}

export function updateLockdownState(
  partial: Partial<import("./types.js").LockdownState>,
) {
  const current = readSettings();
  const lockdownState = {
    ...current.lockdownState,
    ...partial,
  } as import("./types.js").LockdownState;
  writeSettings({ ...current, lockdownState });
}

export function updateAvatarSettings(partial: Partial<AvatarSettings>) {
  const current = readSettings();
  writeSettings({ ...current, avatar: { ...current.avatar, ...partial } });
}
