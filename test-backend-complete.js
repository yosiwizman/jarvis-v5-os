...
async function runTests() {
  ...
  try {
    ...
    if (res.status === 200 && res.data && res.data.ok && res.data.stats) {
      ...
      console.log(`   Total: ${res.data.stats.totalConversations}, Messages: ${res.data.stats.totalMessages}`);
      passedTests++;
    } else {
  ...
  try {
    ...
    if (res.status === 200 && res.data && res.data.ok && Array.isArray(res.data.conversations)) {
      console.log('✅ Retrieved conversation list');
      console.log(`   Total conversations: ${res.data.total}, Returned: ${res.data.conversations.length}`);
      passedTests++;
    } else {
  ...
  if (newConversationId) {
    ...
    try {
      const res = await makeRequest('GET', `/api/conversations/${newConversationId}`);
      if (res.status === 200 && res.data && res.data.ok && res.data.conversation) {
  ...
  try {
    ...
    if (res.status === 200 && res.data && res.data.ok && Array.isArray(res.data.actions)) {
      console.log('✅ Retrieved action list');
      console.log(`   Total actions: ${res.data.total}, Returned: ${res.data.actions.length}`);
      passedTests++;
    } else {
  ...
  try {
    ...
    if (res.status === 200 && res.data && res.data.ok && Array.isArray(res.data.notifications)) {
      console.log('✅ Notifications history retrieved');
      console.log(`   Total notifications: ${res.data.total}, Returned: ${res.data.notifications.length}`);
      passedTests++;
    } else {
